...
class SqliteNotifierRepository(NotifierRepository):
    ...
    def _create_tables(self):
        ...
        self.logger.debug(f"Ensuring SQLite tables exist for " f"Notifier Repository in {self._db.db_path}...")
        sql_statements = [
            """
    ...
    def _deserialize_config(self, adapter_type: NotificationAdapter, config_json: str) -> NotificationConfig:
        ...
        if adapter_type not in NOTIFIER_CONFIG_TYPE_MAP:
            raise NotifierConfigurationError(f"Error reading Notifir configuration. Invalid type '{adapter_type}'")

        config_class: NotificationConfig = NOTIFIER_CONFIG_TYPE_MAP.get(adapter_type)
        if not config_class:
            raise NotifierConfigurationError(f"Error creating Notifir configuration. Type '{adapter_type}'")
