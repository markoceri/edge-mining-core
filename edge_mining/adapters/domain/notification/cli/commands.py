...
def handle_notifier_configuration(
    ...
    if adapter_type == NotificationAdapter.DUMMY:
        return handle_notifier_dummy_config()
...
def handle_add_notifier(configuration_service: ConfigurationService, logger: LoggerPort) -> Optional[Notifier]:
    """Menu to add a new notifier."""
    click.echo(click.style("\n--- Add Notifier ---", fg="yellow"))
    name: str = click.prompt("Name of the notifier", type=str)
    adapter_type: NotificationAdapter = select_notifier_adapter()
    ...
    config: NotificationConfig = handle_notifier_configuration(adapter_type=new_notifier.adapter_type)
    if config is None:
    ...
    if needed_external_service:
        ...
        if external_services:
            ...
            if add_external_service:
                ...
                return None
    try:
        added: Notifier = configuration_service.add_notifier(
...
def handle_list_notifiers(configuration_service: ConfigurationService, logger: LoggerPort) -> None:
    """List all notifiers."""
...
def handle_manage_notifier(configuration_service: ConfigurationService, logger: LoggerPort) -> str:
...
def notifier_menu(configuration_service: ConfigurationService, logger: LoggerPort) -> str:
