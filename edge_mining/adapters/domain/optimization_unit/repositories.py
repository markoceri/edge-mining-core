...
class InMemoryOptimizationUnitRepository(EnergyOptimizationUnitRepository):
    ...
    def __init__(self, initial_units: Optional[Dict[EntityId, EnergyOptimizationUnit]] = None):
        self._optimization_units: Dict[EntityId, EnergyOptimizationUnit] = copy.deepcopy(initial_units) if initial_units else {}
...
class SqliteOptimizationUnitRepository(EnergyOptimizationUnitRepository):
    ...
    def _row_to_optimization_unit(self, row: sqlite3.Row) -> Optional[EnergyOptimizationUnit]:
        ...
        try:
            ...
            return EnergyOptimizationUnit(
                ...
                home_forecast_provider_id=EntityId(row["home_forecast_provider_id"]) if row["home_forecast_provider_id"] else None,
                performance_tracker_id=EntityId(row["performance_tracker_id"]) if row["performance_tracker_id"] else None,
    ...
    def add(self, optimization_unit: EnergyOptimizationUnit) -> None:
        ...
        try:
            ...
            raise OptimizationUnitAlreadyExistsError(f"Optimization Unit with ID {optimization_unit.id} or name '{optimization_unit.name}' already exists: {e}") from e
        ...
        try:
            ...
            with conn:
                ...
                if cursor.rowcount == 0:
                    raise OptimizationUnitNotFoundError(f"No optimization unit found with ID {optimization_unit.id} for update.")
        except sqlite3.Error as e:
    ...
    def remove(self, optimization_unit_id: EntityId) -> None:
        ...
        try:
            with conn:
                ...
                cursor.execute(sql, (optimization_unit_id,))
                if cursor.rowcount == 0:
                    self.logger.warning(f"Attempt to remove non-existent optimization unit with ID {optimization_unit_id}.")
