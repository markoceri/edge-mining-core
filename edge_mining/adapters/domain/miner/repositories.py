...
from edge_mining.domain.miner.exceptions import (
    MinerError, MinerControllerError,
    MinerControllerNotFoundError, MinerControllerAlreadyExistsError,
...
class InMemoryMinerRepository(MinerRepository):
    ...
    def get_by_controller_id(self, controller_id: EntityId) -> List[Miner]:
        ...
        return [
            copy.deepcopy(m) for m in self._miners.values()
            if m.controller_id == controller_id
        ] if controller_id else []
...
class InMemoryMinerControllerRepository(MinerControllerRepository):
    ...
    def __init__(self, initial_miner_controllers: Optional[Dict[EntityId, MinerController]] = None):
        self._miner_controllers: Dict[EntityId, MinerController] = copy.deepcopy(initial_miner_controllers) if initial_miner_controllers else {}
    ...
    def get_by_external_service_id(self, external_service_id: EntityId) -> List[MinerController]:
        ...
        return [
            copy.deepcopy(mc) for mc in self._miner_controllers.values()
...
class SqliteMinerControllerRepository(MinerControllerRepository):
    ...
    def _deserialize_config(
        ...
        config_class: MinerControllerConfig = MINER_CONTROLLER_CONFIG_TYPE_MAP.get(adapter_type)
        if not config_class:
            raise MinerControllerConfigurationError(
    ...
    def add(self, miner_controller: MinerController) -> None:
        ...
        try:
            ...
            # Could mean that the ID already exists
            raise MinerControllerAlreadyExistsError(f"Miner Controller with ID {miner_controller.id} already exists or constraint violation: {e}") from e
        except sqlite3.Error as e:
    ...
    def update(self, miner_controller: MinerController) -> None:
        ...
        try:
            ...
            with conn:
                ...
                if cursor.rowcount == 0:
                    raise MinerControllerNotFoundError(f"No miner controller found with ID {miner_controller.id} for update.")
        except sqlite3.Error as e:
    ...
    def remove(self, miner_controller_id: EntityId) -> None:
        ...
        try:
            with conn:
                ...
                cursor.execute(sql, (miner_controller_id,))
                if cursor.rowcount == 0:
                    self.logger.warning(f"Attempt to remove non-existent miner controller with ID {miner_controller_id}.")
                    # There is no need to raise an exception here, removing a non-existent is idempotent.
        except sqlite3.Error as e:
    ...
    def get_by_external_service_id(self, external_service_id: EntityId) -> List[MinerController]:
        ...
        try:
            ...
            self.logger.error(f"SQLite error getting miner controllers by external service ID {external_service_id}: {e}")
            return []
        finally:
            if conn:
                conn.close()
