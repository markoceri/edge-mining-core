...
def handle_add_miner(
    ...
    hash_rate_max: float = click.prompt(
        "Max HashRate (eg. 100.0)", type=float, default=100.0
    )
    hash_rate_unit: str = click.prompt(
        "HashRate unit (eg. TH/s, GH/s)", type=str, default="TH/s"
    )
    power_consumption_max: float = click.prompt(
        "Max power consumption (Watt, eg. 3200.0)", type=float, default=3200.0
    )
    ...
    if miner_controllers:
        ...
        add_miner_controller: bool = click.confirm(
            "Do you want to add a Miner Controller now?", default=True, abort=False
        )
        ...
        if add_miner_controller:
            ...
            click.echo(
                click.style(
                    "No miner controller configured for this miner.", fg="yellow"
                )
            )
    ...
    try:
        ...
        click.echo(
            click.style(
                f"Miner '{added.name}' (ID: {added.id}) successfully added.", fg="green"
            )
        )
...
def update_single_miner(
    ...
    name: str = click.prompt(
        "New name of the miner", type=str, default=selected_miner.name
    )
    ...
    if miner_controllers:
        ...
        if miner_controller:
            ...
            click.echo(
                click.style("Miner Controller will not be changed!", fg="yellow")
            )
...
def assing_controller_to_miner(
    ...
    try:
        ...
        click.echo(
            click.style(f"Error assigning controller to miner: {e}", fg="red"), err=True
        )
        return None
...
def handle_manage_miner(
    ...
    choice = manage_single_miner_menu(
        miner=selected_miner, configuration_service=configuration_service, logger=logger
    )
...
def print_miner_details(
    ...
    click.echo(
        "| Status: "
        + click.style(
            miner.status.name, fg="green" if miner.status == MinerStatus.ON else "red"
        )
    )
    click.echo(
        "| Max HashRate: "
    ...
    click.echo(
        "| Active: " + click.style(miner.active, fg="green" if miner.active else "red")
    )
    click.echo(
        "| Controller ID: "
        + (str(miner.controller_id) if miner.controller_id else "None")
    )
    ...
    if miner.controller_id:
        ...
        if controller:
            ...
            click.echo(
                "| Controller ID: "
                + click.style(str(miner.controller_id), fg="red")
                + " (not found)"
            )
...
def manage_single_miner_menu(
    miner: Miner, configuration_service: ConfigurationService, logger: LoggerPort
) -> str:
    ...
    while True:
        ...
        if choice == "1":
            try:
                ...
                click.echo(
                    click.style(f"Error activating miner: {e}", fg="red"), err=True
                )
            continue
            ...
            try:
                ...
                click.echo(
                    click.style(f"Error deactivating miner: {e}", fg="red"), err=True
                )
            continue
            ...
            miner = (
                updated_miner or miner
            )  # Update miner if it was successfully updated
            continue
...
def select_miner_controller_type() -> Optional[MinerControllerAdapter]:
    ...
    if (
        not choice.isdigit()
        or int(choice) < 0
        or int(choice) >= len(MinerControllerAdapter)
    ):
        click.echo(click.style("Invalid index. Aborting selection.", fg="red"))
        return None

    controller_type_values = [
        controller_type.value for controller_type in MinerControllerAdapter
    ]
...
def handle_miner_controller_dummy_config(miner: Miner) -> MinerControllerConfig:
    ...
    default_hash_rate = (
        miner.hash_rate_max.value if miner and miner.hash_rate_max else 90.0
    )
    default_hash_rate_unit = (
        miner.hash_rate_max.unit if miner and miner.hash_rate_max else "TH/s"
    )

    power_max: float = click.prompt(
        "Max power consumption (Watt, eg. 3200.0)", type=float, default=default_power
    )
...
def handle_add_miner_controller(
    ...
    needed_external_service = MINER_CONTROLLER_TYPE_EXTERNAL_SERVICE_MAP.get(
        adapter_type, None
    )
    ...
    if needed_external_service:
        ...
        if external_services:
            ...
            if external_service:
                new_controller.external_service_id = (
    ...
    try:
        ...
        click.echo(
            click.style(f"Error adding miner controller: {e}", fg="red"), err=True
        )
    click.pause("Press any key to return to the menu...")
...
def print_miner_controller_details(
    ...
    if show_miner_list:
        ...
        if not miners:
            click.echo(
                click.style("No miners assigned to this controller.", fg="yellow")
            )
        else:
...
def update_single_miner_controller(
    ...
    try:
        ...
        logger.info(
            f"Miner Controller '{updated_controller.name}' (ID: {updated_controller.id}) successfully updated."
        )
    except Exception as e:
        logger.error(f"Error updating miner controller: {e}")
        click.echo(
            click.style(f"Error updating miner controller: {e}", fg="red"), err=True
        )
...
def delete_single_miner_controller(
    ...
    try:
        removed_controller = configuration_service.remove_miner_controller(
            controller_id=controller.id
        )
        logger.info(
            f"Miner Controller '{removed_controller.name}' (ID: {removed_controller.id}) successfully removed."
        )
    except Exception as e:
        logger.error(f"Error removing miner controller: {e}")
        click.echo(
            click.style(f"Error removing miner controller: {e}", fg="red"), err=True
        )
        return False
    else:
        return True
...
def manage_single_miner_controller_menu(
    ...
    while True:
        ...
        print_miner_controller_details(
            controller, configuration_service, show_miner_list=True
        )
        ...
        if choice == "1":
            ...
            controller = (
                updated_controller or controller
            )  # Update controller if it was successfully updated
            continue
...
def select_miner_controller(
    ...
    controller_idx: str = click.prompt(
        "Choose a Controller index", type=str, default=default_idx
    )
    ...
    if (
        not controller_idx.isdigit()
        or int(controller_idx) < 0
        or int(controller_idx) >= len(controllers)
    ):
        click.echo(click.style("Invalid index. Aborting selection.", fg="red"))
        return None
    ...
    while True:
        ...
        if choice == "1":
            ...
            handle_add_miner_controller(
                miner=None, configuration_service=configuration_service, logger=logger
            )
