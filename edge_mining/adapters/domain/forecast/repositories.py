...
class SqliteForecastProviderRepository(ForecastProviderRepository):
    ...
    def _create_tables(self):
        ...
        self.logger.debug(f"Ensuring SQLite tables exist for " f"Forecast Provider Repository in {self._db.db_path}...")
        sql_statements = [
            """
    ...
    def _deserialize_config(self, adapter_type: ForecastProviderAdapter, config_json: str) -> ForecastProviderConfig:
        ...
        config_class: ForecastProviderConfig = FORECAST_PROVIDER_CONFIG_TYPE_MAP.get(adapter_type)
        if not config_class:
            raise ForecastProviderConfigurationError(
