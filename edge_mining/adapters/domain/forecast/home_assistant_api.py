...
class HomeAssistantForecastProviderFactory(ForecastAdapterFactory):
    ...
    def create(
        ...
        builder = HomeAssistantForecastProviderBuilder(
            home_assistant=external_service,
            logger=logger
        )
        ...
        if forecast_provider_config.entity_forecast_energy_next_24h:
            builder.set_next_24h_energy_entity(
                forecast_provider_config.entity_forecast_energy_next_24h,
                forecast_provider_config.unit_forecast_energy_next_24h
            )
...
class HomeAssistantForecastProviderBuilder():
    ...
    def __init__(self, home_assistant: ServiceHomeAssistantAPI, logger: LoggerPort):
        ...
        self.entity_forecast_energy_next_24h: Optional[str] = None
        ...
        self.unit_forecast_energy_actual_h: str = "Wh"
        self.unit_forecast_energy_next_1h: str = "Wh"
        self.unit_forecast_energy_next_24h: str = "Wh"
        self.unit_forecast_energy_remaining_today: str = "Wh"
    ...
    def set_next_24h_energy_entity(
        self,
        entity_id: str,
        unit: str = "kWh"
    ) -> 'HomeAssistantForecastProviderBuilder':
        """Sets the entity ID for the next 24 hours solar energy forecast."""
        self.entity_forecast_energy_next_24h = entity_id
        self.unit_forecast_energy_next_24h = unit.lower()
        return self
    ...
    def build(self) -> 'HomeAssistantForecastProvider':
        ...
        forecast_provider =  HomeAssistantForecastProvider(
            ...
            entity_forecast_energy_next_24h=self.entity_forecast_energy_next_24h,
            ...
            unit_forecast_energy_next_24h=self.unit_forecast_energy_next_24h,
...
class HomeAssistantForecastProvider(ForecastProviderPort):
    ...
    def __init__(
        ...
        entity_forecast_energy_next_24h: Optional[str],
        ...
        unit_forecast_energy_actual_h: str = "Wh",
        unit_forecast_energy_next_1h: str = "Wh",
        unit_forecast_energy_next_24h: str = "Wh",
        unit_forecast_energy_remaining_today: str = "Wh",
        ...
        self.entity_forecast_energy_next_24h = entity_forecast_energy_next_24h
        ...
        self.unit_forecast_energy_next_24h = unit_forecast_energy_next_24h.lower()
        ...
        self.logger.debug(f"Entities Configured for Energy:"
                    ...
                    f"Next 1h='{entity_forecast_energy_next_1h}', "
                    f"Next 24h='{entity_forecast_energy_next_24h}', "
        ...
        self.logger.debug(f"Units Configured for Energy:"
                    ...
                    f"Next 24h='{unit_forecast_energy_next_24h}', "
    ...
    def get_forecast(self) -> Optional[ForecastData]:
        ...
        # --- Next Energy 24h ---
        if self.entity_forecast_energy_next_24h:
            state_forecast_energy_next_24h, _ = self.home_assistant.get_entity_state(
                self.entity_forecast_energy_next_24h
            )
            energy_next_24h = self.home_assistant.parse_energy(
                state_forecast_energy_next_24h,
                self.unit_forecast_energy_next_24h,
                self.entity_forecast_energy_next_24h or "N/A"
            )
        else:
            energy_next_24h = None
        ...
        if energy_next_24h is None and self.entity_forecast_energy_next_24h:
            self.logger.error(f"Missing critical value: Solar Production "
                            f"(Entity: {self.entity_forecast_energy_next_24h})")
            has_critical_error = True

        # Add here other checks for critical values as needed

        if has_critical_error:
            self.logger.error("Failed to retrieve one or more critical energy values "
