...
from edge_mining.domain.energy.value_objects import (
    EnergyStateSnapshot, Battery, Grid, BatteryState, LoadState, GridState
)
    ...
    def get_current_energy_state(self) -> Optional[EnergyStateSnapshot]:
        ...
        if self.storage:
            ...
            if net_power > 0 and self.current_soc < 100.0: # Charging
                ...
                discharge_power = min(
                    abs(net_power),
                    self.storage_max_discharging_power
        ...
        if self.logger:
            self.logger.debug(f"DummyMonitor: Generated state: Prod={production:.0f}W,"
...
class DummySolarEnergyMonitorFactory(EnergyMonitorAdapterFactory):
    ...
    def create(
        ...
        builder = DummySolarEnergyMonitorBuilder(
            logger=logger
        )

        if not isinstance(config, EnergyMonitorDummySolarConfig):
            raise ValueError("Invalid configuration type for Dummy Solar energy monitor. "
