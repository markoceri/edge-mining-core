...
from edge_mining.domain.energy.exceptions import (
    EnergyMonitorError, EnergySourceError, EnergySourceConfigurationError,
    EnergySourceAlreadyExistsError, EnergySourceNotFoundError,
...
class InMemoryEnergySourceRepository(EnergySourceRepository):
    ...
    def __init__(self, initial_energy_sources: Optional[Dict[EntityId, EnergySource]] = None):
        self._energy_sources: Dict[EntityId, EnergySource] = copy.deepcopy(initial_energy_sources) if initial_energy_sources else {}
...
class SqliteEnergySourceRepository(EnergySourceRepository):
    ...
    def add(self, energy_source: EnergySource) -> None:
        ...
        try:
            ...
            raise EnergySourceAlreadyExistsError(f"Energy Source with ID {energy_source.id} already exists or constraint violation: {e}") from e
        except sqlite3.Error as e:
...
class InMemoryEnergyMonitorRepository(EnergyMonitorRepository):
    ...
    def __init__(self, initial_energy_monitors: Optional[Dict[EntityId, EnergyMonitor]] = None):
        self._energy_monitors: Dict[EntityId, EnergyMonitor] = copy.deepcopy(initial_energy_monitors) if initial_energy_monitors else {}
    ...
    def get_by_external_service_id(self, external_service_id: EntityId) -> List[EnergyMonitor]:
        ...
        return [copy.deepcopy(em) for em in self._energy_monitors.values() if em.external_service_id == external_service_id]
...
class SqliteEnergyMonitorRepository(EnergyMonitorRepository):
    ...
    def add(self, energy_monitor: EnergyMonitor) -> None:
        ...
        try:
            ...
            # Could mean that the ID already exists
            raise EnergySourceAlreadyExistsError(f"Energy Monitor with ID {energy_monitor.id} already exists or constraint violation: {e}") from e
        except sqlite3.Error as e:
