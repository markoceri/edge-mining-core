...
class MqttEnergyMonitor(EnergyMonitorPort):
    ...
    def _on_disconnect(self, client, userdata, rc, properties=None):
        ...
        self._connected.clear()
        logger.warning(f"Disconnected from MQTT broker (rc: {rc}). Reconnection should be attempted automatically by Paho.")
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            if internal_name:
                ...
                if internal_name in ["solar_production", "house_consumption", "grid_power", "battery_power"]:
                ...
                if parsed_value is not None:
                    with self._lock:
                        ...
                        logger.debug(f"Stored '{internal_name}' = {parsed_value} (Timestamp: {self._last_update_times[internal_name]})")
                else:
    ...
    def _parse_power(self, state: Optional[str], configured_unit: str, entity_id_for_log: str) -> Optional[Watts]:
        ...
        try:
            ...
            if configured_unit == "kw": value *= 1000
            elif configured_unit != "w": logger.warning(f"Unsupported unit '{configured_unit}' for topic '{entity_id_for_log}'. Assuming Watts.")
            return Watts(value)
    ...
    def get_current_energy_state(self) -> Optional[EnergyStateSnapshot]:
        ...
        def get_value(name: str) -> Tuple[Optional[Any], bool]:
            ...
            if self.topics_map.get(name): # Solo se il topic è configurato
                if value is None:
                    ...
                    logger.warning(f"Data for sensor '{name}' is stale (Last update: {last_update}, Age: {now - last_update if last_update else 'N/A'})")
                    is_stale = True
                    stale = True
        ...
        if is_stale:
             logger.warning("One or more sensor values are stale. Using last known values, but they might be inaccurate.")
        ...
        if battery_soc is not None and battery_power is not None and self.battery_capacity is not None:
             ...
             logger.debug("Battery SOC topic configured, but full BatteryState cannot be created (missing power topic/value or static capacity setting?).")
        ...
        logger.info(f"MQTT Monitor: State Snapshot: Prod={snapshot.production:.0f}W, "
