...
def select_external_service_type() -> Optional[ExternalServiceAdapter]:
    ...
    if (
        not choice.isdigit()
        or int(choice) < 0
        or int(choice) >= len(ExternalServiceAdapter)
    ):
        click.echo(click.style("Invalid index. Aborting selection.", fg="red"))
        return None

    es_type_values = [
        controller_type.value for controller_type in ExternalServiceAdapter
    ]
...
def handle_add_external_service(
    ...
    if adapter_type is None:
        click.echo(
            click.style("Invalid external service type selected. Aborting.", fg="red")
        )
        return None
...
def select_external_service(
    ...
    choice: str = click.prompt(
        "Choose an external service by index", type=str, default=default_idx
    )
    choice = choice.strip().lower()
    if choice == "b":
        return None
...
def print_external_service_details(
    ...
    if show_linked_instances:
        external_service_linked_entities = (
...
def update_single_external_service(
    ...
    try:
        ...
        click.echo(
            click.style(f"Error updating external service: {e}", fg="red"), err=True
        )
...
def delete_single_external_service(
    ...
    try:
        removed_external_service = configuration_service.remove_external_service(
            service.id
        )
        click.echo(
            click.style(
        ...
        click.echo(
            click.style(f"Error removing external service: {e}", fg="red"), err=True
        )
        return False
    else:
        return True
...
def manage_single_external_service_menu(
    ...
    while True:
        ...
        print_external_service_details(
            selected_service, configuration_service, show_linked_instances=True
        )
...
def external_services_menu(
    ...
    while True:
        ...
        if choice == "1":
            ...
            if service is None:
                click.echo(
                    click.style("No external service selected. Aborting.", fg="red")
                )
                continue
