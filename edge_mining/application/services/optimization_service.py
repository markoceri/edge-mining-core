...
class OptimizationService:
    ...
    async def _notify_unit(self, notifiers: List[NotificationPort], title: str, message: str):
        ...
        for notifier in notifiers:
            try:
                ...
                if not success:
                    if self.logger:
                        self.logger.warning(f"Notifier {type(notifier).__name__} "
    ...
    async def run_all_enabled_units(self):
        ...
        if self.logger:
            self.logger.info(f"Optimization run for all units finished. "
                            f"{len(enabled_units)} units processed.")

    async def _process_unit(self, optimization_unit: EnergyOptimizationUnit):
        if self.logger:
            self.logger.info(f"Processing Optimization Unit: "
        ...
        if not policy:
            if self.logger:
                self.logger.error(f"Policy ID {optimization_unit.policy_id} for optimization unit '{optimization_unit.name}' not found. Skipping.")
            return
        else:
            if self.logger:
                self.logger.info(f"Optimization unit '{optimization_unit.name}' > "
        ...
        if not energy_source:
            if self.logger:
                self.logger.error(f"Energy source for optimization unit '{optimization_unit.name}' "
        ...
        if not energy_monitor:
            if self.logger:
                self.logger.error(f"Energy monitor for energy source '{energy_source.name}' "
        ...
        if not forecast_provider:
            if self.logger:
                self.logger.warning(f"Forecast provider for energy source '{energy_source.name}' "
        ...
        if optimization_unit.home_forecast_provider_id:
            home_forecast_provider = self.adapter_service.get_home_load_forecast_provider(optimization_unit.home_forecast_provider_id)
        # Home forecast provider is optional, so log a warning if it's missing but continue
        if not home_forecast_provider:
            if self.logger:
                self.logger.warning(f"Home forecast provider for optimization unit '{optimization_unit.name}' (Config ID: {optimization_unit.home_forecast_provider_id}) not found. Skipping forecast provier.")
        ...
        if optimization_unit.performance_tracker_id:
            mining_performance_tracker = self.adapter_service.get_mining_performace_tracker(optimization_unit.performance_tracker_id)
        # Mining performance tracker is optional, so log a warning if it's missing but continue
        if not mining_performance_tracker:
            if self.logger:
                self.logger.warning(f"Mining performance tracker for optimization unit '{optimization_unit.name}' "
                                    f"(Config ID: {optimization_unit.performance_tracker_id}) not found. "
                                    f"Skipping mining performace tracker.")
        ...
        try:
            ...
            if not energy_state:
                if self.logger:
                    self.logger.error(f"Could not retrieve energy state for optimization unit '{optimization_unit.name}'. Skipping.")
                await self._notify_unit(unit_notifiers, f"Optimizer Error ({optimization_unit.name})", "Failed to retrieve energy state.")
                return
        except Exception as e:
            if self.logger:
                self.logger.error(f"Error getting energy state for optimization unit '{optimization_unit.name}': {e}")
            await self._notify_unit(unit_notifiers, f"Optimizer Error ({optimization_unit.name})", f"Energy state error: {e}")
            return
        ...
        if forecast_provider:
            ...
            if self.logger:
                self.logger.info(f"No solar forecast provider configured for optimization unit '{optimization_unit.name}'.")
        ...
        if home_forecast_provider:
            try:
                ...
                if self.logger:
                    self.logger.warning(f"Error getting home load forecast for optimization unit '{optimization_unit.name}': {e}")
        else:
            if self.logger:
                self.logger.info(f"No home load forecast provider configured for optimization unit '{optimization_unit.name}'.")
        ...
        if mining_performance_tracker:
            try:
                # TODO: Provide parameters if needed
                tracker_current_hashrate = mining_performance_tracker.get_current_hashrate()
            except Exception as e:
                if self.logger:
                    self.logger.warning(f"Error getting mining performance tracker for optimization unit '{optimization_unit.name}': {e}")
        ...
        if self.logger:
            self.logger.info(f"Finished processing for optimization unit '{optimization_unit.name}'. {len(miner_processing_tasks)} miners controlled.")
    ...
    async def _process_single_miner_in_unit(
        ...
        if not miner:
            if self.logger:
                self.logger.error(f"Miner {miner_id} in optimization unit "
        ...
        if miner.controller_id:
            miner_controller = self.adapter_service.get_miner_controller(miner.controller_id)
            if not miner_controller:
                if self.logger:
                    self.logger.error(f"Controller for miner {miner_id} "
        ...
        try:
            ...
            await self._execute_miner_decision(miner_controller, miner_id, decision, current_status, notifiers, optimization_unit.name)

        except (MinerError, PolicyError) as e:
            if self.logger:
                self.logger.error(f"Domain error processing miner {miner_id} in optimization unit '{optimization_unit.name}': {e}", exc_info=True)
            await self._notify_unit(notifiers, f"Optimizer Error ({optimization_unit.name} / {miner_id})", f"Domain error: {e}")
        except Exception as e: # Other exceptions (e.g. IO from the controller)
            if self.logger:
                self.logger.error(f"Unexpected error processing miner {miner_id} in optimization unit '{optimization_unit.name}': {e}", exc_info=True)
            await self._notify_unit(notifiers, f"Optimizer Error ({optimization_unit.name} / {miner_id})", f"Runtime error: {e}")

    async def _execute_miner_decision(
        self, controller: MinerControlPort, miner_id: EntityId, decision: MiningDecision,
        current_status: MinerStatus, notifiers: List[NotificationPort], unit_name: str
    ):
        action_taken = False
        success = False
        ...
        if decision == MiningDecision.START_MINING and current_status != MinerStatus.ON:
            ...
            success = controller.start_miner(miner_id)
            action_taken = True
            if success:
                await self._notify_unit(notifiers, f"Miner Started: {miner_id}", f"Miner {miner_id} was started." + message_suffix)
            else:
                await self._notify_unit(notifiers, f"Miner Start Failed: {miner_id}", f"Attempt to start miner {miner_id} failed." + message_suffix)
            ...
            success = controller.stop_miner(miner_id)
            action_taken = True
            if success:
                await self._notify_unit(notifiers, f"Miner Stopped: {miner_id}", f"Miner {miner_id} was stopped." + message_suffix)
            else:
                await self._notify_unit(notifiers, f"Miner Stop Failed: {miner_id}", f"Attempt to stop miner {miner_id} failed." + message_suffix)

        if action_taken and not success:
            if self.logger:
                self.logger.error(f"Command {decision.name} for miner {miner_id} failed using controller {type(controller).__name__}.")
